class Hero {
  ...
  constructor() {
    ...
    this.isStickyTicking = false; // Add this for throttling sticky header updates
  ...
  isChromeiOS() {
    ...
    return /iPad|iPhone|iPod/.test(userAgent) || /Chrome/.test(userAgent);
  }
...
class HeroApp {
  ...
  handleScroll() {
    ...
    if (!this.hero.isStickyTicking) {
      this.hero.isStickyTicking = true;
      requestAnimationFrame(() => {
        if (this.hero) {
          this.hero.manageStickyHeader();
          this.hero.isStickyTicking = false;
        }
      });
    }
  }
